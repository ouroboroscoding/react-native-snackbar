...
const MAX_WIDTH: number = 340;

// Only one Snackbar is allowed in any app, we keep track of it here
let Instance: Snackbar | null = null;

/**
 * Generic types
 */
type SnackbarMessageActionOnPress = () => void;
 ...
 * @returns void
 */
export function addMessage(options: string | SnackbarMessage): void {
...
export class Snackbar extends React.Component<SnackbarProps, SnackbarState> {
    ...
    action(id: string, callback: SnackbarMessageActionOnPress) {
    ...
    addMessage(options: string | SnackbarMessage): void {
        ...
        if(typeof options === 'string') {
            ...
            const message: SnackbarMessageState = {
                _id: id,
                text: options.text
            }
            if(options.action) {
    ...
    remove(id: string) {
        const index: number = afindi(this.state.messages, '_id', id);
        if(index > -1) {
    ...
    render() {
        ...
        const scontainer: StyleProp<ViewStyle> = {zIndex: this.props.zIndex}
        const smessage: StyleProp<ViewStyle> = {}
        ...
        return (
            <View style={[styles.container, scontainer]}>
                {this.state.messages.map((message: SnackbarMessageState) =>
                    <View
                        key={message._id}
                        style={[this.props.messageStyle, styles.message, smessage]}
                    >
